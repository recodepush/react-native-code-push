...
- (instancetype)init {
  ...
  if (!serverURL) {
    serverURL = @"https://server-recodepush.nihontechhub.com/";
  }
  ...
  if (buildVersion)
    [_configDictionary setObject:buildVersion forKey:BuildVersionConfigKey];
  if (serverURL)
    [_configDictionary setObject:serverURL forKey:ServerURLConfigKey];
  if (clientUniqueId)
    [_configDictionary setObject:clientUniqueId forKey:ClientUniqueIDConfigKey];
  if (deploymentKey)
...
// no setter for PublicKey, because it's need to be hard coded within Info.plist
// for safety
